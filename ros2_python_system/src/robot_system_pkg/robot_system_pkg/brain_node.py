...
class BrainNode(Node):
    ...
    def make_movement_decisions(self)->Tuple[float, float]:
        ...
        elif self.front_us > us_threshold and self.front_ir == 1 and self.left_ir == 1 and self.right_ir == 0:
            self.get_logger().info(f"small left obstacle, moving with velocity {0} and radian {-0.78}")
            return 0.0, -0.78
        
        # if small right obstacle, turn left
        elif self.front_us > us_threshold and self.front_ir == 1 and self.left_ir == 0 and self.right_ir == 1:
