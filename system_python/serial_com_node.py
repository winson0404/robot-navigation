...
from custom_robot_interface.msg import RCStatus     # Custom message, from external robot controller (RC).
from custom_robot_interface.msg import RCCommand    # Custom message, to external robot controller.
...
class SerialComNode(Node):  # Create a class, inherited from the Node class.
    ...
    def __init__(self):     # Constructor.
        super().__init__("serial_com_node") # Create a node, initialize node name. 
                            ...
                            # sibling class.
        self.get_logger().info("Serial COM node is starting up...")
        
        # Create a publisher using the interface RCStatus,
        # with topic "tpc_RC_Status", and queue size of 8.
    ...
    2. Acknowledgement from RC when receiving a command packet from ROS2. This is just 1 byte,
       either ACK or NAK. (Not implemented for now)
    """
    def Timer_Callback(self):   
        if self.sport.is_open == True:
            bytetoread = self.sport.in_waiting
            if bytetoread > DATA_PACKET_LENGTH:  # At least equals to number of bytes in one data packet.
                ...
                if (sum == 0) or (sum == 256): # Make sure checksum is correct.
                    if ReadData[0] == 0x62:    # Compare with character 'b'.
                        ...
                        # Sample codes to assemble 4 bytes into a 32-bits signed integer
                        # Direct:
                        ...
                        # Using built-in method in 'int' class:
                        ...
                        # Get heading. Convert the bytes into 16-bits signed integer.
