...
namespace comms
{
    ...
    Socket::Socket(const int port, const char *host) : port(port), _socket(INVALID_SOCKET), host(host)
    {
    ...
    bool Socket::Send(SOCKET clientSocket, const char *data, int data_length)
    {
        int bytesSent = send(clientSocket, data, data_length, 0);
        return bytesSent == data_length;
    }

    bool Socket::Receive(SOCKET clientSocket, char *data, int data_length)
    {
